...
Mineradio (KG 适配版) — 基于 [XxHuberrr/Mineradio](https://github.com/XxHuberrr/Mineradio) 改编，音源统一为酷狗（KG），免登录即可使用全功能。

- 原作者：[XxHuberrr](https://github.com/XxHuberrr)
- 原始仓库：`https://github.com/XxHuberrr/Mineradio.git`
- 当前分支：KG 单源适配版
- 当前版本：`v1.1.1`
...
- 音源已统一为酷狗（KG），不要再引入 QQ 音乐、网易云音乐或其他音源平台代码。
...
Mineradio 是 Windows Electron 桌面音乐播放器，基于酷狗（KG）音源，核心体验包括搜索、播放、歌单、歌词、3D 歌单架、粒子视觉预设、DIY 视觉控制台和 GitHub 自动更新。
...
├─ build/               # 打包资源和 installer 脚本
├─ docs/                # 设计文档
├─ server.js            # 本地 API、KG 音乐源、更新检查
├─ lx-source-bridge.js  # LX 音源桥接框架（KG 接入核心）
├─ dj-analyzer.js       # 节奏/音频分析
├─ package.json         # 版本号、构建命令、electron-builder 配置
