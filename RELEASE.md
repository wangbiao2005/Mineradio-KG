...
- 确认 `node_modules/`、`dist/`、`.cookie`、`updates/` 没有进入 git。
- 运行语法检查：`git diff --check`、`node --check server.js`。
...
应用会请求 GitHub Releases latest 检测新版本。远端版本高于本地版本时，应用内更新入口会展示 Release 内容并引导下载安装。

本地验证更新链路时，可以通过 `MINERADIO_UPDATE_MANIFEST` 环境变量指向本地 manifest JSON 来模拟线上 Release。
# 发布流程
