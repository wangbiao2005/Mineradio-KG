// ====================================================================
//  粒子音乐可视化播放器 — Server v3 (KG Only)
//  - 酷狗(KG)搜索 / 歌曲URL / 封面/音频代理
//  - 无需登录，全平台 KG 音源
// ====================================================================
const { loadSourceDirectory, getMusicUrl, getMusicLyric, getMusicPic, getLoadedSources } = require('./lx-source-bridge');
...
var userCookie = null; // KG-only 模式，不使用网易/QQ Cookie
...
process.on('uncaughtException', (err) => {
  if (err && err.code === 'EADDRINUSE') { console.error('[Server] 端口被占用，无法启动'); process.exit(1); }
  console.warn('[Server] Uncaught Exception:', err && err.message || err);
});
...
// ---------- 本地歌单/收藏存储 ----------
const LOCAL_PLAYLIST_FILE = process.env.LOCAL_PLAYLIST_FILE || path.join(__dirname, 'local-playlists.json');
let localPlaylistData = { playlists: [], likedSongs: [] };
(function loadLocalPlaylists() {
  try {
    if (fs.existsSync(LOCAL_PLAYLIST_FILE)) {
      const raw = fs.readFileSync(LOCAL_PLAYLIST_FILE, 'utf8');
      const parsed = JSON.parse(raw);
      localPlaylistData.playlists = Array.isArray(parsed.playlists) ? parsed.playlists : [];
      localPlaylistData.likedSongs = Array.isArray(parsed.likedSongs) ? parsed.likedSongs : [];
      console.log('[LocalPlaylist] 加载完成: ' + localPlaylistData.playlists.length + ' 个歌单, ' + localPlaylistData.likedSongs.length + ' 首喜欢');
    }
  } catch (e) { console.warn('[LocalPlaylist] 加载失败:', e.message); }
})();
function saveLocalPlaylists() {
  try { fs.writeFileSync(LOCAL_PLAYLIST_FILE, JSON.stringify(localPlaylistData, null, 2)); } catch (e) {}
}
function localSongKey(song) {
  if (!song) return '';
  return [song.source || song.provider || '', song.songmid || song.mid || song.id || '', song.name || ''].filter(Boolean).join(':');
}
...
function normalizeQualityPreference(value) {
  ...
  if (['jymaster', 'master', 'studio', 'svip'].includes(raw)) return 'hires';
...
function lxQualityFromPlayback(value) {
  const raw = normalizeQualityPreference(value);
  // 音源脚本只支持 128k / 320k，flac 已注释不可用
  if (raw === 'hires' || raw === 'lossless') return '320k';
  if (raw === 'exhigh') return '320k';
  return '128k';
}

function lxDurationMs(value) {
  const n = Number(value) || 0;
  if (!n) return 0;
  return n > 10000 ? n : n * 1000;
}

function lxSongKey(source, item) {
  return String(item.songmid || item.mid || item.musicrid || item.id || item.hash || item.copyrightId || '')
    || `${source}:${item.name || ''}:${item.artist || item.singer || ''}`;
}

function mapLXSong(source, item) {
  item = item || {};
  const sourceKey = source || item.source || item.provider || 'kw';
  const rawMusicRid = String(item.musicrid || item.MUSICRID || '');
  let songmid = String(item.songmid || item.mid || item.id || rawMusicRid || item.hash || '');
  if (sourceKey === 'kw') songmid = songmid.replace(/^MUSIC_/i, '');
  const name = item.name || item.SONGNAME || item.songname || item.title || '';
  const artist = item.artist || item.singer || item.ARTIST || item.singername || '';
  const album = item.album || item.albumName || item.ALBUM || '';
  const albumId = item.albumId || item.albumID || item.ALBUMID || '';
  const hash = item.hash || item.FileHash || item.fileHash || '';
  const copyrightId = item.copyrightId || item.copyrightID || '';
  const cover = item.cover || item.img || item.pic || item.albumPic || '';
  return {
    provider: sourceKey,
    source: sourceKey,
    type: sourceKey,
    lx: true,
    id: songmid || hash || copyrightId,
    songmid,
    mid: songmid,
    musicrid: rawMusicRid || (sourceKey === 'kw' && songmid ? 'MUSIC_' + songmid : ''),
    hash,
    copyrightId,
    albumId,
    albumMid: item.albumMid || item.album_mid || albumId,
    mediaMid: item.mediaMid || item.media_mid || item.strMediaMid || '',
    strMediaMid: item.strMediaMid || item.mediaMid || item.media_mid || '',
    name,
    artist,
    artists: artist ? artist.split(/\s*[/、,，]\s*/).filter(Boolean).map(n => ({ name: n })) : [],
    album,
    cover,
    duration: lxDurationMs(item.duration || item.interval || item._interval),
    fee: 0,
    playable: true,
    sourceLabel: LX_PLATFORM_LABELS[sourceKey] || sourceKey.toUpperCase(),
    hasLossless: !!item.hasLossless || !!(item._types && (item._types.flac || item._types.flac24bit)),
  };
}
...
function uniqueLXSongs(songs, limit) {
  ...
  for (const song of songs || []) {
    ...
    const key = `${song.source}:${lxSongKey(song.source, song)}`;
...
async function fetchJsonText(url, opts) {
  ...
  try { return JSON.parse(text); } catch (_) {
    const cleaned = text.replace(/^\s*callback\(/, '').replace(/\);\s*$/, '');
...
async function fetchKgHotSearch() {
  ...
  try {
    ...
    return list.map(item => item.keyword || item.keywords || '').filter(Boolean);
  } catch (e) {
    console.warn('[KgHotSearch] failed:', e.message);
    return [];
  }
}
...
async function handleSearch(keywords, limit) {
  console.log('[Search] KG:', keywords, 'limit:', limit);
  try {
    var songs = await searchKugou(keywords, Math.max(4, Math.min(50, limit || 20)));
    return songs;
  } catch(e) {
    console.error('[Search] KG failed:', e.message);
    return [];
  }
}

async function handleDiscoverHome() {
  // KG 无需登录，多源推荐：热搜词 → 3组不同歌曲
  try {
    // 1. 获取 KG 实时热搜词
    var hotTerms = [];
    try {
      hotTerms = await fetchKgHotSearch();
    } catch(e) { /* fallback to hardcoded */ }
    
    // 2. 确保至少有 3 个不同关键词
    if (hotTerms.length < 3) {
      hotTerms = hotTerms.concat(['热歌', '新歌', '流行', '抖音', '经典', '民谣', '电音', '说唱']);
    }
    
    // 3. 用不同热搜词搜索，给不同卡片使用
    var kw1 = hotTerms[0] || '热歌';
    var kw2 = hotTerms[1] || '新歌';
    var kw3 = hotTerms[2] || '流行';
    console.log('[DiscoverHome] KG 热搜: ' + kw1 + ', ' + kw2 + ', ' + kw3);
    
    var [dailySongs, trendingSongs, newSongs] = await Promise.all([
      searchKugou(kw1, 12),
      searchKugou(kw2, 12),
      searchKugou(kw3, 12),
    ]);
    ...
    return {
      loggedIn: false,
      user: null,
      songs: dailySongs,         // 向后兼容
      dailySongs: dailySongs,    // → 每日推荐
      trendingSongs: trendingSongs,  // → 私人电台
      newSongs: newSongs,        // → 常听歌手
      hotTerms: hotTerms.slice(0, 5),
      playlists: [],
      podcasts: [],
      mode: 'kg',
      updatedAt: Date.now(),
    ...
    console.error('[DiscoverHome] KG failed:', e.message);
    return {
      loggedIn: false,
      user: null,
      songs: [], dailySongs: [], trendingSongs: [], newSongs: [],
      playlists: [], podcasts: [],
      mode: 'kg',
      updatedAt: Date.now(),
    };
...
async function handleSongUrl(id, loginInfo, qualityPreference) {
  ...
  try {
    ...
    console.log('[SongUrl] KG failed:', e.message);
  }
  return { url: null, trial: false, playable: false, reason: 'unavailable', message: 'KG source not available' };
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/song/url') {
    try {
      ...
      if (lxUrl && /^https?:/.test(lxUrl)) {
        ...
        sendJSON(res, { url: null, playable: false, level: null, quality: 'none' });
      }
    } catch (err) {
      console.error('[SongUrl]', err);
      sendJSON(res, { url: null, playable: false, error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/cover') {
    try {
      ...
      const referer = coverUrl.includes('kugou.com') ? 'https://www.kugou.com/'
        : coverUrl.includes('kuwo.cn') ? 'https://www.kuwo.cn/'
        : coverUrl.includes('migu.cn') ? 'https://music.migu.cn/'
        : 'https://music.163.com/';
      const resp = await fetch(coverUrl, { headers: { 'User-Agent': UA, 'Referer': referer } });
  ...
  if (pn === '/api/lx-search') {
    try {
      ...
      if (!keywords.trim()) { sendJSON(res, { ok: false, error: 'Missing keywords', songs: [] }, 400); return; }

      const songs = await searchLXMusic(source, keywords, limit);
      sendJSON(res, { ok: true, source, total: songs.length, songs });
    } catch (err) {
  ...
  if (pn === '/api/lx-song-url') {
    try {
      ...
      const copyrightId = url.searchParams.get('copyrightId') || url.searchParams.get('cid') || '';
      const albumId = url.searchParams.get('albumId') || url.searchParams.get('aid') || '';
      const hash = url.searchParams.get('hash') || '';
      const mediaMid = url.searchParams.get('mediaMid') || url.searchParams.get('strMediaMid') || '';
      const name = url.searchParams.get('name') || '';
      const artist = url.searchParams.get('artist') || '';
      const album = url.searchParams.get('album') || '';

      if (!songId && !songMid && !hash) {
        sendJSON(res, { ok: false, error: 'Missing song id/mid/hash', url: null }, 400);
        return;
      }
      ...
      sendJSON(res, {
        ...
        source,
        quality,
        level: normalizeQualityPreference(quality),
        url: songUrl,
        proxyUrl: songUrl ? `/api/audio?url=${encodeURIComponent(songUrl)}` : null,
      });
    } catch (err) {
      console.error('[LX SongUrl]', err.stack || err.message);
      sendJSON(res, { ok: false, error: err.message, url: null }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/lx-lyric') {
    try {
      const source = url.searchParams.get('source') || url.searchParams.get('platform') || 'kw';
      const songId = url.searchParams.get('id') || url.searchParams.get('songmid') || '';
      const songMid = url.searchParams.get('songmid') || url.searchParams.get('mid') || songId;
      const copyrightId = url.searchParams.get('copyrightId') || url.searchParams.get('cid') || '';
      const albumId = url.searchParams.get('albumId') || url.searchParams.get('aid') || '';
      ...
      let lyricInfo;
      try {
        lyricInfo = await getMusicLyric(source, {
          songmid: songMid,
          id: songId,
          copyrightId,
          albumId,
        ...
        // LX 歌词 handler 失败，对 kg/kw/mg 源走原生 API fallback
        if (source === 'kg' && hash) {
          console.log('[LX Lyric] LX handlers 失败，使用 kg 原生歌词 API');
          const kgLyric = await fetchKgLyric(hash, 0);
          lyricInfo = { lyric: kgLyric, tlyric: '', rlyric: '', lxlyric: '' };
        } else if (source === 'kw' && songMid) {
          console.log('[LX Lyric] LX handlers 失败，使用 kw 原生歌词 API');
          const kwLyric = await fetchKwLyric(songMid);
          lyricInfo = { lyric: kwLyric, tlyric: '', rlyric: '', lxlyric: '' };
        } else if (source === 'mg' && copyrightId) {
          console.log('[LX Lyric] LX handlers 失败，使用 mg 原生歌词 API');
          const mgLyric = await fetchMgLyric(copyrightId);
          lyricInfo = { lyric: mgLyric, tlyric: '', rlyric: '', lxlyric: '' };
        } else {
          throw lxErr;
        }
      }
      sendJSON(res, { ok: true, source, ...lyricInfo });
    } catch (err) {
      console.warn('[LX Lyric]', err.message);
      sendJSON(res, { ok: false, error: err.message, lyric: '' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/local/playlists') {
    try {
      const list = localPlaylistData.playlists.map(pl => ({
        id: pl.id, name: pl.name, trackCount: (pl.songs || []).length,
        cover: (pl.songs && pl.songs.length > 0 && pl.songs[0].cover) || '',
        createdAt: pl.createdAt, updatedAt: pl.updatedAt,
        source: 'local',
      }));
      sendJSON(res, { ok: true, playlists: list });
  ...
  if (pn === '/api/local/playlist/create') {
    try {
      ...
      if (!name) { sendJSON(res, { ok: false, error: 'Missing name' }, 400); return; }
      const pl = {
      ...
      localPlaylistData.playlists.push(pl);
      saveLocalPlaylists();
      sendJSON(res, { ok: true, playlist: { id: pl.id, name: pl.name, trackCount: 0, cover: '', source: 'local' } });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
    return;
  }
  ...
  if (pn === '/api/local/playlist/add-song') {
    try {
      ...
      const pid = String(body.pid || body.playlistId || '');
      if (!pid) { sendJSON(res, { ok: false, error: 'Missing pid' }, 400); return; }
      const pl = localPlaylistData.playlists.find(p => p.id === pid);
      if (!pl) { sendJSON(res, { ok: false, error: 'Playlist not found' }, 404); return; }
      const songEntry = {
        id: body.id || '',
        songmid: body.songmid || body.mid || body.id || '',
        name: body.name || '',
        artist: body.artist || '',
        album: body.album || '',
        cover: body.cover || '',
      ...
      if (!songEntry.name) { sendJSON(res, { ok: false, error: 'Missing song name' }, 400); return; }
      const dupKey = localSongKey(songEntry);
      if (pl.songs.some(s => localSongKey(s) === dupKey)) {
        sendJSON(res, { ok: true, success: true, message: '歌曲已在歌单中' });
        return;
      }
      pl.songs.push(songEntry);
      pl.updatedAt = Date.now();
      saveLocalPlaylists();
      sendJSON(res, { ok: true, success: true, trackCount: pl.songs.length });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
    return;
  }
  ...
  if (pn === '/api/local/playlist/remove-song') {
    try {
      ...
      const songId = String(body.songId || body.id || '');
      if (!pid || !songId) { sendJSON(res, { ok: false, error: 'Missing pid or songId' }, 400); return; }
      const pl = localPlaylistData.playlists.find(p => p.id === pid);
      if (!pl) { sendJSON(res, { ok: false, error: 'Playlist not found' }, 404); return; }
      pl.songs = pl.songs.filter(s => String(s.id) !== songId && String(s.songmid) !== songId);
      pl.updatedAt = Date.now();
      saveLocalPlaylists();
      sendJSON(res, { ok: true, success: true, trackCount: pl.songs.length });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
  ...
  if (pn === '/api/local/playlist/tracks') {
    try {
      ...
      if (!pid) { sendJSON(res, { ok: false, error: 'Missing id' }, 400); return; }
      const pl = localPlaylistData.playlists.find(p => p.id === pid);
      if (!pl) { sendJSON(res, { ok: false, error: 'Playlist not found' }, 404); return; }
      sendJSON(res, { ok: true, playlist: { id: pl.id, name: pl.name, trackCount: pl.songs.length }, tracks: pl.songs });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
    return;
  }

  // ---------- 本地喜欢 ----------
  if (pn === '/api/local/like') {
    try {
      const body = req.method === 'POST' ? await readBody(req) : Object.fromEntries(url.searchParams);
      const like = String(body.like || 'true').toLowerCase() !== 'false';
      const songEntry = {
        id: body.id || '',
        songmid: body.songmid || body.mid || body.id || '',
        name: body.name || '',
      ...
      const idx = localPlaylistData.likedSongs.findIndex(s => localSongKey(s) === key);
      if (like && idx === -1) {
        localPlaylistData.likedSongs.unshift({ ...songEntry, likedAt: Date.now() });
      } else if (!like && idx !== -1) {
        localPlaylistData.likedSongs.splice(idx, 1);
      }
      saveLocalPlaylists();
  ...
  if (pn === '/api/local/like/check') {
    try {
      ...
      const liked = {};
      ids.forEach(id => {
        const found = localPlaylistData.likedSongs.some(s => String(s.id) === id || String(s.songmid) === id);
        liked[id] = found;
      });
      sendJSON(res, { ok: true, liked });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
    return;
  }
  ...
  if (pn === '/api/local/liked') {
    try {
      sendJSON(res, { ok: true, songs: localPlaylistData.likedSongs });
    } catch (err) { sendJSON(res, { ok: false, error: err.message }, 500); }
    return;
  }
...
// ---------- 酷我搜索 ----------
function searchKuwo(keywords, limit = 12) {
  return new Promise((resolve, reject) => {
    const q = encodeURIComponent(keywords);
    const apiUrl = `https://search.kuwo.cn/r.s?all=${q}&ft=music&itemset=web_2013&client=kt&pn=0&rn=${limit}&rformat=json&encoding=utf8`;
    https.get(apiUrl, { headers: { 'User-Agent': 'okhttp/4.10.0' } }, (res) => {
      let data = '';
      ...
      res.on('end', () => {
        try {
          ...
          const songs = abslist.map(item => ({
            id: String(item.MUSICRID || '').replace('MUSIC_', ''),
            musicrid: item.MUSICRID || '',
            name: String(item.NAME || '').replace(/&nbsp;/g, ' ').trim(),
            artist: String(item.ARTIST || '').replace(/&nbsp;/g, ' ').trim(),
            album: String(item.ALBUM || '').replace(/&nbsp;/g, ' ').trim(),
            albumId: item.ALBUMID || '',
            cover: item.web_albumpic_short ? (item.web_albumpic_short.startsWith('http') ? item.web_albumpic_short : 'https://img3.kuwo.cn/star/albumcover/240/' + item.web_albumpic_short) : '',
            duration: parseInt(item.DURATION) || 0,
            provider: 'kw',
            source: 'kw',
            type: 'kw',
            playable: true,
            hasLossless: (item.FORMATS || '').includes('FLAC') || (item.FORMATS || '').includes('APE'),
          }));
          // 排序：原版优先，伴奏/Live/演唱会靠后
          songs.sort((a, b) => {
            const aLow = (a.name + a.album).toLowerCase();
            const bLow = (b.name + b.album).toLowerCase();
            const aBad = aLow.includes('伴奏') || aLow.includes('live') || aLow.includes('演唱会') || aLow.includes('ktv');
            const bBad = bLow.includes('伴奏') || bLow.includes('live') || bLow.includes('演唱会') || bLow.includes('ktv');
          ...
          try {
            ...
            const songs = abslist.map(item => ({
              ...
              cover: item.web_albumpic_short ? (item.web_albumpic_short.startsWith('http') ? item.web_albumpic_short : 'https://img3.kuwo.cn/star/albumcover/240/' + item.web_albumpic_short) : '',
              duration: parseInt(item.DURATION) || 0,
              provider: 'kw', source: 'kw', type: 'kw', playable: true,
              hasLossless: (item.FORMATS || '').includes('FLAC') || (item.FORMATS || '').includes('APE'),
            }));
            songs.sort((a, b) => {
              const aLow = (a.name + a.album).toLowerCase();
            ...
            reject(new Error('Kuwo search parse error'));
          }
        }
      });
...
// ---------- 加载 LX 音源 ----------
function fetchKgSongInfo(hash) {
  return new Promise((resolve) => {
    if (!hash) { resolve(null); return; }
    const url = `https://m.kugou.com/app/i/getSongInfo.php?cmd=playInfo&hash=${hash}`;
    https.get(url, { timeout: 5000, headers: { 'User-Agent': 'Mozilla/5.0 (Linux; Android 11) AppleWebKit/537.36 Mobile Safari/537.36', Referer: 'https://www.kugou.com/' }, rejectUnauthorized: false }, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => {
...
function kgCoverUrlFromInfo(info) {
  if (!info) return '';
  const img = info.album_img || info.imgUrl || info.album_cover || '';
  if (!img) return '';
  // 强制 HTTPS
  let url = img.replace(/\{size\}/g, '240');
  if (url.startsWith('http://')) url = url.replace('http://', 'https://');
  return url;
}

async function fetchKgLyric(hash, durationSec) {
  if (!hash) return '';
  const timelength = Math.max(1, Math.round(Number(durationSec) || 0)) * 1000;
  const url = `http://m.kugou.com/app/i/krc.php?cmd=100&hash=${hash}&timelength=${timelength}`;
  try {
    const resp = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0', Referer: 'https://www.kugou.com/' } });
    return await resp.text() || '';
  } catch (e) { return ''; }
}

// ---------- 咪咕原生歌词 ----------
async function fetchMgLyric(copyrightId) {
  if (!copyrightId) return '';
  const url = `https://music.migu.cn/v3/api/music/audioPlayer/getLyric?copyrightId=${encodeURIComponent(copyrightId)}`;
  try {
    const json = await fetchJsonText(url, {
      headers: {
    ...
    console.warn('[MG Lyric]', e.message);
    return '';
  }
}

// ---------- 酷我原生歌词 (newlyric XOR + zlib) ----------
const KW_LYRIC_KEY = Buffer.from('yeelion');
function buildKwLyricParams(musicrid) {
  const rid = String(musicrid || '').replace(/^MUSIC_/i, '');
  const params = `user=12345,web,web,web&requester=localhost&req=1&rid=MUSIC_${rid}`;
  const bufStr = Buffer.from(params);
  const bufStrLen = bufStr.length;
  const keyLen = KW_LYRIC_KEY.length;
  const output = new Uint16Array(bufStrLen);
  for (let i = 0; i < bufStrLen; i++) {
    output[i] = KW_LYRIC_KEY[i % keyLen] ^ bufStr[i];
  }
  return Buffer.from(output).toString('base64');
}

async function fetchKwLyric(musicrid) {
  if (!musicrid) return '';
  const rid = String(musicrid).replace(/^MUSIC_/i, '');
  const encryptedParams = buildKwLyricParams(rid);
  const url = `http://newlyric.kuwo.cn/newlyric.lrc?${encryptedParams}`;
  try {
    ...
    if (bodyStart < 4) return '';
    const zlib = require('zlib');
    const inflated = await new Promise((resolve, reject) => {
      zlib.inflate(buf.subarray(bodyStart), (err, result) => {
        if (err) reject(err);
        else resolve(result);
      });
    });
    const iconv = require('iconv-lite');
    return iconv.decode(inflated, 'gb18030');
  } catch (e) {
    console.warn('[KW Lyric]', e.message);
    return '';
  }
}

async function searchKugou(keywords, limit = 12) {
  const q = encodeURIComponent(keywords);
  const apiUrl = `http://mobilecdn.kugou.com/api/v3/search/song?format=json&keyword=${q}&page=1&pagesize=${limit}&showtype=10&plat=0&version=9108&tag=1&correct=1&privilege=1&sver=5`;
  const json = await fetchJsonText(apiUrl, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (Linux; Android 11) AppleWebKit/537.36 Mobile Safari/537.36',
      Referer: 'https://www.kugou.com/',
    },
  ...
  return list.map((item) => {
    // 封面: 优先 trans_param.union_cover (搜索API自带, 不依赖限流接口)
    var cover = '';
    var unionCover = item.trans_param && item.trans_param.union_cover;
    if (unionCover) {
      cover = unionCover.replace(/\{size\}/g, '240');
    } else if (item.imgurl) {
      cover = item.imgurl.includes('{size}') ? item.imgurl.replace(/\{size\}/g, '240') : item.imgurl;
    }
    if (cover && cover.startsWith('http://')) cover = cover.replace('http://', 'https://');
    return mapLXSong('kg', {
      id: item.audio_id || item.Audioid,
      songmid: item.audio_id || item.Audioid,
      hash: item.hash || item.FileHash,
      name: item.songname || item.SongName || item.filename,
      ...
      _types: {
        '128k': item.hash,
        '320k': item['320hash'] || item['320filesize'],
        flac: item.sqhash || item.sqfilesize,
      },
    });
  }).filter(song => song.name);
}

async function searchMigu(keywords, limit = 12) {
  const q = encodeURIComponent(keywords);
  const apiUrl = `https://app.c.nf.migu.cn/MIGUM2.0/v1.0/content/search_all.do?isCopyright=1&isCorrect=1&pageNo=1&pageSize=${limit}&searchSwitch=%7B%22song%22%3A1%2C%22album%22%3A0%2C%22singer%22%3A0%2C%22tagSong%22%3A0%2C%22mvSong%22%3A0%2C%22songlist%22%3A0%2C%22bestShow%22%3A0%7D&sort=0&text=${q}`;
  const json = await fetchJsonText(apiUrl, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (Linux; Android 11) AppleWebKit/537.36 Mobile Safari/537.36',
  ...
  const raw = json && (json.songResultData && (json.songResultData.result || json.songResultData.resultList) || json.songResult && json.songResult.result || json.songResult || []);
  const list = Array.isArray(raw) ? raw : [];
  return list.map(item => mapLXSong('mg', {
    id: item.songId || item.id,
    songmid: item.songId || item.id,
    copyrightId: item.copyrightId || item.copyrightID,
    name: item.name || item.songName,
    artist: item.singers ? item.singers.map(s => s.name).join(' / ') : (item.singer || item.singerName),
    album: item.album || item.albumName || (item.albums && item.albums[0] && item.albums[0].name),
    albumId: item.albumId || item.albumID || (item.albums && item.albums[0] && item.albums[0].id),
    duration: item.duration,
    cover: item.imgItems && item.imgItems[0] && item.imgItems[0].img || item.img3 || item.img2 || item.img1 || '',
    _types: { '128k': true, '320k': true, flac: true },
  })).filter(song => song.name && (song.songmid || song.copyrightId));
}

async function searchTencentAsLX(keywords, limit = 12) {
  // 使用 QQ 音乐公网搜索 API (u.y.qq.com)，无需登录
  const dataParam = encodeURIComponent(JSON.stringify({
    req: {
      method: 'DoSearchForQQMusicDesktop',
      module: 'music.search.SearchCgiService',
      param: { searchid: 3, query: keywords, grp: 1, num: limit, highlight: 1 }
    }
  }));
  const apiUrl = `https://u.y.qq.com/cgi-bin/musicu.fcg?format=json&data=${dataParam}`;
  try {
    const json = await fetchJsonText(apiUrl, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', Referer: 'https://y.qq.com/' },
    });
    if (!json || json.code !== 0) throw new Error('QQ API返回异常');
    const list = (json.req && json.req.data && json.req.data.body && json.req.data.body.song && json.req.data.body.song.list) || [];
    return list.map(item => mapLXSong('tx', {
      id: item.mid,
      songmid: item.mid,
      name: item.name || item.title,
      ...
      cover: item.album && item.album.mid ? `https://y.qq.com/music/photo_new/T002R300x300M000${item.album.mid}.jpg` : '',
    }));
  } catch (err) {
    console.warn('[TX Search] 公网API失败:', err.message);
    return [];
  }
}

async function searchNeteaseAsLX(keywords, limit = 12) {
  // 使用网易云公网搜索 API，无需登录
  const q = encodeURIComponent(keywords);
  const apiUrl = `https://music.163.com/api/search/get?s=${q}&type=1&limit=${limit}&offset=0`;
  try {
    const json = await fetchJsonText(apiUrl, {
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', Referer: 'https://music.163.com/' },
    });
    const list = (json && json.result && json.result.songs) || [];
    return list.map(item => mapLXSong('wy', {
      id: item.id,
      songmid: item.id,
      name: item.name,
      artist: Array.isArray(item.artists) ? item.artists.map(a => a.name).join('、') : (item.artistsName || ''),
      album: item.album ? (item.album.name || '') : (item.albumname || ''),
      albumId: item.album ? (item.album.id || '') : (item.albumid || ''),
      duration: Math.round((item.duration || 0) / 1000),
      cover: item.album && item.album.picUrl ? item.album.picUrl : '',
    }));
  } catch (err) {
    console.warn('[WY Search] 公网API失败:', err.message);
    return [];
  }
}
...
(async () => {
  try {
    await loadSourceDirectory(LX_SOURCE_DIR);
    const sources = getLoadedSources();
    const keys = Object.keys(sources);
    if (keys.length > 0) {
      console.log('[LX Source] 已加载 ' + keys.length + ' 个音源: ' + keys.join(', '));
    } else {
      console.log('[LX Source] 音源目录为空或加载失败，将仅使用内置音源');
    }
  } catch (err) {
